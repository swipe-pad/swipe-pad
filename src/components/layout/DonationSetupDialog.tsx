...
export function DonationSetupDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="surface-panel-strong max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-surface-border bg-surface-2/95 px-4 py-3 backdrop-blur-xl">
          ...
          <button onClick={onClose} className="rounded-md p-1 text-muted-foreground transition-colors hover:text-white" aria-label="Close donation setup">
            <X className="size-5" />
          </button>
        </div>
